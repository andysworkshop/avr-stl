...
inline size_t __deque_buf_size(size_t __size) {
  return __size < avrstl::AvrDequeBufferSize() ? size_t(avrstl::AvrDequeBufferSize() / __size) : size_t(1);
}
#else
inline size_t __deque_buf_size(size_t __size) {
...
#ifdef __AVR__
  if ((unsigned long)__elemsbefore < __length / 2) {				// ptrdiff_t = signed on AVR (long int)
#else
